...
for i = 1:iters
  ...
  for j = 1:l3Dim
    convdelta4(:, :, j) = rowToMat(convdelta4Row(1, ((i - 1) * 4 * 4 + 1):(i * 4 * 4)), 4, 4);
  end
  ...
  % Back propagate the gradients with respect to convTheta2. This can be done by convoluting the layer before the weights with the errors of the layers after the weights
  for j = 1:l1Dim
    for k = 1:l3Dim 
      filter = convdelta3(:, :, k);
  ...
  for j = 1:l3Dim
    convBiasDelta(j) = sum(sum(convdelta3(:, :, j)));
  end
